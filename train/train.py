...
from pilot.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN
from torch.utils.data import Dataset
from pilot.train.pilot_trainer import pilotTrainer

from pilot import conversation as conversation_lib
from pilot.model import *
from pilot.mm_utils import tokenizer_image_token
...
class ModelArguments:
    ...
    Diff_loss_coef: Optional[float] = field(default=0.0)
...
def safe_save_model_for_hf_trainer(trainer: transformers.Trainer,
    ...
    if getattr(trainer.args, "tune_mm_mlp_adapter", False):
        ...
        parent_folder = os.path.dirname(output_dir)  # lol output dir
...
def train():
    ...
    # if model_args.scales is not None:  # lol
    ...
    if model_args.vision_tower is not None:
        if 'mpt' in model_args.model_name_or_path:
            ...
            model = pilotMPTForCausalLM.from_pretrained(
            ...
            model = pilotLlamaForCausalLM.from_pretrained(
                model_args.model_name_or_path,
                cache_dir=training_args.cache_dir,
                **bnb_model_from_pretrained_args
            )
    ...
    model.config.training = True  # lol
    model.config.mlp_smoe = model_args.mlp_smoe  # lol
    ...
    if model_args.vision_tower is not None:
        ...
        # lol freeze moe expert
        for i in range(model.config.num_experts):
            if i > 0:
                for f in model.get_model().mm_projector.experts[i].parameters():
    ...
    data_module = make_supervised_data_module(tokenizer=tokenizer,
                                              data_args=data_args)
    trainer = pilotTrainer(model=model,
                    tokenizer=tokenizer,
                    args=training_args,
                    **data_module)
